...
class ProjectController {
    ...
    try {
      ...
      if (
        ...
        throw new ValidationError(
          "All fields are required including project image"
        );
      }
      ...
      // Process image
      const imageBase64 = await FileUploadService.processImageToBase64(
        req.file
      );
  ...
  static updateProject = asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        project.engID.toString() !== sessionUser.id
      ) {
      ...
      if (req.file) {
        updateData.image = await FileUploadService.processImageToBase64(
          req.file
        );
      }
   ...
   */
  static getFeaturedProjects = asyncHandler(async (req, res) => {
    const { limit = 6 } = req.query;

    try {
      // Get featured projects (you can modify this logic based on your requirements)
      const projects = await Project.find({
        isActive: true,
      })
        .populate("engineerId", "firstName lastName name profilePicture")
        .sort({ createdAt: -1 })
        .limit(parseInt(limit))
        .lean();
      ...
      const formattedProjects = projects.map((project) => ({
        ...
        engineer: {
          _id: project.engineerId._id,
          name:
            project.engineerId.name ||
            `${project.engineerId.firstName} ${project.engineerId.lastName}`,
          profilePicture: project.engineerId.profilePicture,
        },
